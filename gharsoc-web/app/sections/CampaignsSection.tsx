...
export default function CampaignsSection() {
  ...
  const handleDelete = async (id: string) => { if (!confirm('Delete?')) return; await fetch(`/api/campaigns?id=${id}`, { method: 'DELETE' }); fetchData() }
  ...
  return (
    <div className="space-y-4">
        ...
        <Dialog open={showAdd} onOpenChange={setShowAdd}>
          ...
          <DialogContent className="max-w-md">
            ...
            <div className="space-y-3 mt-2">
              ...
              <div><Label className="text-xs">Script Instructions for Arya</Label><Textarea placeholder="Pitch the new Prestige Lakeside project..." value={form.script_template} onChange={e => setForm(f => ({ ...f, script_template: e.target.value }))} rows={4} /></div>
            </div>
      ...
      : <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {items.map(c => (
          <div key={c._id} className="border border-border rounded-xl bg-card p-5">
            <div className="flex items-start justify-between">
              <div>
                <h3 className="font-semibold text-foreground flex items-center gap-2">{c.name} <Badge variant={c.status === 'active' ? 'default' : 'secondary'} className="text-[10px]">{c.status}</Badge></h3>
